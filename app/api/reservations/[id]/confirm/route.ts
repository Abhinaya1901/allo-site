  ...
  try {
    const updated = await prisma.$transaction(async (tx) => {
      const rows = await tx.$queryRaw
        { id: string; stockId: string; quantity: number; status: string; expiresAt: Date }[]
      >`SELECT id, "stockId", quantity, status, "expiresAt" FROM "Reservation" WHERE id = ${id} FOR UPDATE`;
      ...
      await tx.stock.update({ where: { id: r.stockId }, data: { totalUnits: { decrement: r.quantity }, reservedUnits: { decrement: r.quantity } } });
